...
def CuadradoNumerico(fich, repetirFila, repetirColumna):
    inicio = time.time()
    hx = 0
    hy = 0
    numerosPosibles = 10 ##Combinacion de número que podemos generar (0-9)
    tab, h = leerFichero(fich)
    BackTracking(tab, h, hx, hy, numerosPosibles, inicio, repetirFila, repetirColumna)
...
def BackTracking(tab, h, hx, hy, numerosPosibles, inicio, repetirFila, repetirColumna):
    solucionEncontrada, hx, hy = EsSolucion(tab,hx,hy, repetirFila, repetirColumna)
    if(solucionEncontrada == True): ## Si encontramos una fila válida
        if(h == 0): ## Y ademas los huecos están completos (es decir, que sean 0)
            ...
            while i < numerosPosibles:
                valor = str(i)
                tab[hx][hy] = valor
                BackTracking(tab, h, hx, hy, numerosPosibles, inicio, repetirFila, repetirColumna) ##Repetimos el proceso
                i+=1
            incognita = "?"
...
def checkTablero(tab, hx, hy, repetirFila, repetirColumna):
    if (repetirFila == False and repetirColumna == False):
        return True
    if(repetirFila == True):
        ListaFila = []
        for i in range(0, len(tab) - 2, 2):
            if(str(tab[hx][i]) != '?'):
                for j in range(len(ListaFila)):
                    if(str(tab[hx][i]) == str(ListaFila[j])):
                        return False
                ListaFila.append(str(tab[hx][i]))
            else:
                break
        return True;
    if(repetirColumna == True):
        ListaColumna = []
        for i in range(0, len(tab) - 2, 2):
            if(str(tab[i][hy]) != '?'):
                for j in range(len(ListaColumna)):
                    if(str(tab[i][hy]) == str(ListaColumna[j])):
                        return False
                ListaColumna.append(str(tab[i][hy]))
            else:
                break
        return True;
...
def EsSolucion(tab, hx, hy, repetirFila, repetirColumna):
    if (checkTablero(tab, hx, hy, repetirFila, repetirColumna) == True):
        filaValida, hx, hy = SaberSiFilaValida(tab, hx, hy)
        if(filaValida == False):
            return False, hx, hy
        else:
            return True, hx, hy
...
def SaberSiFilaValida(tab, hx, hy):
    ...
    return SaberSiColumnaValida(tab, hx, hy) ## Si coincide todas las filas comprobamos columnas
...
##repetirFila = input("¿Los valores de las filas pueden repetirse? (True == No permites, False == Permites)")
##repetirColumna = input("¿Los valores de las columnas pueden repetirse? (True == No permites, False == Permites)")
##CuadradoNumerico(fichero, repetirFila, repetirColumna)

CuadradoNumerico('test01.txt', True, False)