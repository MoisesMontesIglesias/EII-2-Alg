...
def CuadradoNumerico(fich, comprobarFila, comprobarColumna):
    inicio = time.time()
    hx = 0
    hy = 0
    numerosPosibles = 10 ##Combinacion de número que podemos generar (0-9)
    tab, h = leerFichero(fich)
    BackTracking(tab, h, hx, hy, numerosPosibles, inicio, comprobarFila, comprobarColumna)
...
def BackTracking(tab, h, hx, hy, numerosPosibles, inicio, comprobarFila, comprobarColumna):
    solucionEncontrada, hx, hy = EsSolucion(tab,hx,hy, comprobarFila, comprobarColumna)
    if(solucionEncontrada == True): ## Si encontramos una fila válida
        if(h == 0): ## Y ademas los huecos están completos (es decir, que sean 0)
            ...
            while i < numerosPosibles:
                valor = str(i)
                tab[hx][hy] = valor
                BackTracking(tab, h, hx, hy, numerosPosibles, inicio, comprobarFila, comprobarColumna) ##Repetimos el proceso
                i+=1
            incognita = "?"
...
def EsSolucion(tab, hx, hy, comprobarFila, comprobarColumna):
    filaValida, hx, hy = SaberSiFilaValida(tab, hx, hy, comprobarFila, comprobarColumna)
    if(filaValida == False):
        return False, hx, hy
    else:
        return True, hx, hy

def SaberSiFilaValida(tab, hx, hy, comprobarFila, comprobarColumna):
    for i in range(0,len(tab) - 2, 2):
        ListaFila = []
        for j in range(0,len(tab[i]) - 2, 2):
            if(tab[i][j] == "?"): ##Posición sin rellenar
                return True, i, j
        numero = int(tab[i][0]) ## Realizamos la suma de toda la filaç
        ListaFila.append(tab[i][0])
        for j in range(1,len(tab[i]) - 2, 2):
            if(comprobarFila == True):
                if(tab[i][j+1] in ListaFila):
                    return False, hx, hy
                ListaFila.append(tab[i][j+1])
            if(tab[i][j] == "+"):
    ...
    return SaberSiColumnaValida(tab, hx, hy, comprobarColumna) ## Si coincide todas las filas comprobamos columnas

def SaberSiColumnaValida(tab, hx, hy, comprobarColumna):
    for i in range(0,len(tab)-2,2):
        ListaColumna = []
        ListaColumna.append(tab[0][i])
        numero = int(tab[0][i])
        for j in range(1,len(tab[0]) - 2, 2):
            if(comprobarColumna == True):
                if(tab[j+1][i] in ListaColumna):
                    return False, hx, hy
                ListaColumna.append(tab[j+1][i])
            if(tab[j][i] == "+"):
...
CuadradoNumerico('test01.txt', False, False)