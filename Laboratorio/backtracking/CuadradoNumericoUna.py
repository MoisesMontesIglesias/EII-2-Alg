...
def BackTracking(tab, h, hx, hy, numerosPosibles, inicio):
    ...
    if(solucionEncontrada == True): ## Si encontramos una fila válida
        if(h == 0): ## Y ademas los huecos están completos (es decir, que sean 0)
            ...
            sys.exit() ## Permite forzar la finalización del programa y devolver únicamente la primera solución
            return
        else:
...
def EsSolucion(tab, hx, hy):
    filaValida, valorX, valorY = SaberSiFilaValida(tab, hx, hy)
    if(filaValida == False):
        return False, hx, hy
    else:
        return True, valorX, valorY
...
def printMatriz(tab, tiempoSolucion):
    ...
    for i in range(len(tab)):
        ...
        print(cadena)
    print("Fin del programa, ya se encontró la solución que se buscaba")
