...
def main():
    ...
    Lista = numeroPorDefecto.copy()
    CuadradoNumerico(Lista, tamaño, matriz, numeroPorDefecto, resultadoFila, resultadoColumna)
    fin = time.time()
...
def IteradorNumero(NumeroGenerado, numeroPorDefecto, valor):
    NumeroGenerado[valor] = str(int(NumeroGenerado[valor])+1)
    for i in range(len(NumeroGenerado)-1, -1, -1):
        if NumeroGenerado[i] == '10':
            if(i == 0):
                ...
                NumeroGenerado[i-1] = str(int(NumeroGenerado[i-1])+1)
                NumeroGenerado[i] = '0'
...
def CuadradoNumerico(numeroGenerado, tamaño, matriz, numeroPorDefecto, resultadoFila, resultadoColumna):
    for i in range(10**len(numeroPorDefecto)):
        if(NumeroPosible(numeroGenerado, matriz, numeroPorDefecto) == False):
            numeroGenerado = IteradorNumero(numeroGenerado, numeroPorDefecto, tamaño**2-1)
        else:
